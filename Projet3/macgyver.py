...
class Macgyver:
    ...
    @property
    def score(self):
        # To win, bucket`attribute is used to count
        # objects picked up by the main character
    ...
    def move(self, direction, structure_laby):
        ...
        # Direction parameter(string) define the direction wished by user.
    ...
    def apply_move(self, direction, structure_laby, position_future, is_out_laby):
        ...
        if is_out_laby:
            ...
            # if position_future parameter is equal to GARDIAN_LETTER
            # it means that the game is over and we can check
            # if the main character won or not
            # see check_victory_or_defeat
            if not structure_laby[position_future] == WALL_LETTER:
                # take the object and
                # look if the personage position match the object
